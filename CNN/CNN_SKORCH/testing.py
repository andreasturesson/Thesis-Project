...
class Net(nn.Module):
    def __init__(self,
        ...
        #self.fc = nn.Linear(np.sum(num_filters),1024)
        #self.fc_middle = nn.Linear(1024,1024)
    ...
    def forward(self, input_ids):
        ...
        x_fc = F.relu(self.fc(self.dropout(x_fc)))
        
        x_fc = F.relu(self.fc_middle(self.dropout(x_fc)))

        # Compute logits. Output shape: (b, n_classes)
        #logits = self.fc_out(self.dropout(x_fc))
        
        logits = F.sigmoid(self.fc_out(x_fc))
...
def grid_search(net, train_X, train_y):
...
def main():
    ...
    search = False
    if search:
        grid_search(net=net, train_X=train_X, train_y=train_y)
        return
